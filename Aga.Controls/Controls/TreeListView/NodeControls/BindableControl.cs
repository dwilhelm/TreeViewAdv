...
namespace Quimron.WinUI.Controls.TreeListView.NodeControls
{
	public abstract class BindableControl: NodeControl
	{
		...
		public virtual object GetValue(TreeListNode node)
		{
			if (VirtualMode)
			{
		...
		public virtual void SetValue(TreeListNode node, object value)
		{
			if (VirtualMode)
			{
				NodeControlValueEventArgs args = new NodeControlValueEventArgs(node);
		...
		public Type GetPropertyType(TreeListNode node)
		{
			return GetMemberAdapter(node).MemberType;
		}

		private MemberAdapter GetMemberAdapter(TreeListNode node)
		{

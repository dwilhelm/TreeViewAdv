...
namespace Quimron.WinUI.Controls.TreeListView.NodeControls
{
	public abstract class BaseTextControl : EditableControl
	{
		...
		protected Size GetLabelSize(TreeListNode node, DrawContext context)
		{
			CheckThread();
		...
		private void CreateBrushes(TreeListNode node, DrawContext context, out Brush textBrush, out Brush backgroundBrush, out Font font)
		{
		...
		public string GetLabel(TreeListNode node)
		{
		...
		public void SetLabel(TreeListNode node, string value)
		{
			SetValue(node, value);
		}
